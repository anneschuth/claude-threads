 ...
 * (tmp + rename + chmod). session-store, memory/store and github-emails-store
 * still carry hand-rolled copies of this pattern from before the extraction —
 * migrate them here opportunistically so hardening (fsync, tmp cleanup,
 * error policy) lands once instead of per store.
 */
