...
describe('SessionStore', () => {
  ...
  describe('timeoutPostId to lifecyclePostId migration', () => {
    it('migrates timeoutPostId to lifecyclePostId on load', async () => {
      // Create a session with the legacy field name by manually writing to the store
      const session = createTestSession({ threadId: 'legacy-thread' });
      const sessionId = `${session.platformId}:${session.threadId}`;

      // Save normally first
      store.save(sessionId, session);

      // Now manually inject a legacy timeoutPostId into the raw data
      // We access the file directly since there's no public API for this
      const fs = await import('fs');
      const os = await import('os');
      const path = await import('path');
      const sessionsFile = path.join(os.homedir(), '.config', 'claude-threads', 'sessions.json');
      const data = JSON.parse(fs.readFileSync(sessionsFile, 'utf-8'));

      // Add legacy timeoutPostId, remove lifecyclePostId
      data.sessions[sessionId].timeoutPostId = 'legacy-timeout-post-123';
      delete data.sessions[sessionId].lifecyclePostId;

      fs.writeFileSync(sessionsFile, JSON.stringify(data, null, 2));

      // Create a new store instance to trigger fresh load with migration
      const newStore = new SessionStore();
      const loaded = newStore.load();

      // Verify migration happened
      const loadedSession = loaded.get(sessionId);
      expect(loadedSession).toBeDefined();
      expect(loadedSession?.lifecyclePostId).toBe('legacy-timeout-post-123');
      // The legacy field should be removed from the persisted data
      const rawData = JSON.parse(fs.readFileSync(sessionsFile, 'utf-8'));
      expect(rawData.sessions[sessionId].timeoutPostId).toBeUndefined();
      expect(rawData.sessions[sessionId].lifecyclePostId).toBe('legacy-timeout-post-123');
    });

    it('findByPostId finds sessions with legacy timeoutPostId', async () => {
      const fs = await import('fs');
      const os = await import('os');
      const path = await import('path');
      const sessionsFile = path.join(os.homedir(), '.config', 'claude-threads', 'sessions.json');

      // Create a session and manually set legacy field
      const session = createTestSession({ threadId: 'legacy-find-thread' });
      const sessionId = `${session.platformId}:${session.threadId}`;
      store.save(sessionId, session);

      // Inject legacy timeoutPostId
      const data = JSON.parse(fs.readFileSync(sessionsFile, 'utf-8'));
      data.sessions[sessionId].timeoutPostId = 'legacy-post-456';
      delete data.sessions[sessionId].lifecyclePostId;
      fs.writeFileSync(sessionsFile, JSON.stringify(data, null, 2));

      // Find by the legacy post ID
      const newStore = new SessionStore();
      const found = newStore.findByPostId('test-platform', 'legacy-post-456');

      expect(found).toBeDefined();
      expect(found?.threadId).toBe('legacy-find-thread');
      // The returned session should have lifecyclePostId populated
      expect(found?.lifecyclePostId).toBe('legacy-post-456');
    });
  });
});
