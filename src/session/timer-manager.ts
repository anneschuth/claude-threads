...
/**
 * Clear only the update timer.
 */
export function clearUpdateTimer(timers: SessionTimers): void {
  if (timers.updateTimer) {
    clearTimeout(timers.updateTimer);
    timers.updateTimer = null;
  }
...
/**
 * Clear only the typing timer.
 */
export function clearTypingTimer(timers: SessionTimers): void {
  if (timers.typingTimer) {
    clearInterval(timers.typingTimer);
    timers.typingTimer = null;
  }
...
/**
 * Clear only the status bar timer.
 */
export function clearStatusBarTimer(timers: SessionTimers): void {
  if (timers.statusBarTimer) {
    clearInterval(timers.statusBarTimer);
    timers.statusBarTimer = null;
  }
 ...
 */
export function hasActiveTimers(timers: SessionTimers): boolean {
  return timers.updateTimer !== null ||
         timers.typingTimer !== null ||
         timers.statusBarTimer !== null;
}

/**
 * Check if typing timer is running.
 * Useful for UI updates (isTyping state).
 */
export function isTyping(timers: SessionTimers): boolean {
  return timers.typingTimer !== null;
}
