...
describe('pinPost', () => {
  ...
  it('pins a post with correct endpoint', async () => {
    const mockFetch = createMockFetch(() => Promise.resolve({
      ok: true,
      json: () => Promise.resolve({}),
    }));
    ...
    await pinPost(mockConfig, 'post123');
...
describe('unpinPost', () => {
  ...
  it('unpins a post with correct endpoint', async () => {
    const mockFetch = createMockFetch(() => Promise.resolve({
      ok: true,
      json: () => Promise.resolve({}),
    }));
    ...
    await unpinPost(mockConfig, 'post456');

    expect(mockFetch).toHaveBeenCalledWith(
      'https://mattermost.example.com/api/v4/posts/post456/unpin',
      expect.objectContaining({
        method: 'POST',
      })
    );
...
describe('getPinnedPosts', () => {
  ...
  it('returns array of pinned post IDs', async () => {
    const mockFetch = createMockFetch(() => Promise.resolve({
      ok: true,
      json: () => Promise.resolve({
        order: ['post1', 'post2', 'post3'],
        posts: {
          post1: { id: 'post1', channel_id: 'channel1', message: 'First' },
          post2: { id: 'post2', channel_id: 'channel1', message: 'Second' },
          post3: { id: 'post3', channel_id: 'channel1', message: 'Third' },
        },
      }),
    }));
    globalThis.fetch = mockFetch;

    const result = await getPinnedPosts(mockConfig, 'channel1');

    expect(result).toEqual(['post1', 'post2', 'post3']);
    expect(mockFetch).toHaveBeenCalledWith(
      'https://mattermost.example.com/api/v4/channels/channel1/pinned',
      expect.objectContaining({
        method: 'GET',
      })
    );
  ...
  it('returns empty array when order is undefined', async () => {
    const mockFetch = createMockFetch(() => Promise.resolve({
      ok: true,
      json: () => Promise.resolve({
        posts: {},
