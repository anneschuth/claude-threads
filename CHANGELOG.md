...
- **A run of `Bash` commands renders as one self-rewriting line.** Eight commands in a row produced eight blocks — each with a blank line and a `↳ ✓` result line under it — and because the display budget is 50 characters while `cd ~/workspaces/workdir/<project> &&` eats ~40 of them, all eight rendered as the same truncated prefix. The `cd` scaffolding is now stripped so the command itself is visible, and consecutive calls to the same tool collapse into a single line that updates in place: `💻 Bash ×8 glab ci status ✓ (41s)`. The outcome is stamped onto that line instead of adding another one. Any other content (text, a different tool) ends the run, so a single call looks exactly as it did before. Tools opt in via `ToolFormatResult.group`; `Bash` is the only one so far.
