...
- **`read_post` / session-store lookups no longer cross the platform boundary.** `findByThreadIdAnyState` accepts the caller's `platformId` and the plain-reply resume path passes it, so a thread id that collides across platforms can no longer resume another platform's session (its allowlist, working dir, worktree and Claude account).
