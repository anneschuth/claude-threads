...
- **The side-conversation guard now works on Slack.** Slack delivers mentions as raw `<@U0…>` tokens, never `@name`, so the guard (active sessions, paused sessions, and the new DCM path) silently never matched there — every human-to-human aside in a session thread was fed to Claude as a follow-up.
