...
### Added
- **Structured rate-limit detection.** Modern CLIs emit a `rate_limit_event` with `rate_limit_info: {status, resetsAt, rateLimitType}` on every turn. The bot now feeds **`status: "rejected"`** events into the existing account-cooldown path (`resetsAt` epoch-seconds, sanity-bounded to at most 8 days out; slightly-past values from clock skew clamp to a brief cooldown), complementing the stderr phrase-scraping which remains for older CLIs. The predicate is strict equality on `rejected` — the SDK's status union is `allowed | allowed_warning | rejected`, and `allowed_warning` (~70%+ utilization, request went through) must not bench a healthy account: cooldowns only ever extend, so one warning would have parked the account until its weekly reset. A reset-less text-scanner hit is also ignored while a cooldown from a precise structured hit is still running, so the 1-hour default guess can't stretch a known-shorter deadline.
- **The MCP plan-approval prompt now shows the plan.** On modern CLIs `ExitPlanMode` routes through the permission prompt; it used to render as a bare "Permission requested: ExitPlanMode" with no plan content. The prompt now carries the plan text (truncated at 1500 chars). Known remaining conflict, tracked as follow-up: the MCP prompt and the bot's plan-approval UI are two competing approval surfaces — reacting on the bot's UI alone lets the MCP prompt time out and deny the plan.

### Hardening (from two rounds of adversarial review of this change)
- **A resumed session can no longer lose its restored task list.** Task state is in-memory, so after a bot restart the resumed CLI session references task ids the fresh tracker has never seen. A single post-resume `TaskUpdate(completed)` created one completed placeholder, `allCompleted` fired on it, and the executor deleted the correctly-restored task-list post while reporting full completion. `allCompleted` now requires at least one task the tracker actually saw created — placeholder-only sets never signal completion. (Persisting tracker state across restarts, which would also restore full names instead of `Task #N` placeholders, is tracked as follow-up.)
- **Fresh-CLI restarts now await the old process's exit before clearing state.** `kill()` resolves on process exit but wasn't awaited, so the dying CLI (up to ~2s of buffered stream-json) could repopulate the just-cleared task/tool state — recreating the exact id-collision the clear exists to prevent. All three respawn sites (`!cd`, both worktree switches) now await it.
- **Failed-create refreshes use the right action.** Removing a failed `TaskCreate` now emits `complete` when the remainder is empty or fully completed (deleting the pinned post) instead of rendering a lingering empty "0/0" task post; resolving a create that absorbs a placeholder row now refreshes the display immediately (and adopts a rename the early update carried).
- **Task-list updates are coalesced per event.** A parallel burst of N `TaskCreate` calls in one assistant event now yields one task-post update (the final snapshot) instead of N back-to-back `updatePost` calls that could trip platform rate limits.
- **Reset-less rate-limit hits only defer to *precise* deadlines.** The round-1 suppression was too broad: it also swallowed reset-less repeats during a cooldown that itself came from a 1-hour default guess, silently retiring the documented "extend on longer deadline" behavior. Suppression now applies only while a cooldown from an explicit reset time is running.
- **Subagent output can no longer trigger main-session commands.** The sidechain filter is applied in the event handlers too: text from a forwarded subagent event (which the thread never displays) no longer executes `!cd`-style Claude commands against the main session, and subagent tool uses/errors no longer pollute bug-report context. PR-URL detection intentionally still sees subagent text.
- **Plan text in permission prompts is truncation-safe.** The `ExitPlanMode` plan preview cuts on a code-point boundary and re-balances an unclosed ``` fence, so the reaction legend and decision lines can't be swallowed into a code block.
- **Tracker robustness:** numeric `taskId` values are coerced instead of silently freezing the displayed list; multi-MB tool results are no longer copied when no `TaskCreate` is pending; and a non-error create result that doesn't match the expected `"Task #N created"` wording logs a warning — that wording is the one CLI dependency whose drift would silently darken the task display again.
- **Fresh-CLI restarts clear accumulated task state.** `!cd` and worktree switches respawn Claude with a fresh session whose task numbering restarts at #1; stale `TaskTracker` entries would collide with the new ids and corrupt updates. `MessageManager.clearClaudeSessionState()` now runs on every `resume: false` respawn (resume restarts like `!permissions` keep their state, since a resumed session keeps its numbering).
- **Failed `TaskCreate` calls no longer leave ghost tasks.** A create whose tool result is an error (or doesn't carry the expected "Task #N created" text — e.g. a future CLI rewording) removes the task and refreshes the display; previously it stayed forever as an un-updatable pending row that also pinned `allCompleted` at false.
- **Sidechain events are skipped.** Events carrying `parent_tool_use_id` (subagent activity forwarded by some CLI versions) no longer reach the transformer: a subagent's `TaskCreate`/`TaskUpdate` calls would otherwise permanently pollute the main thread's task list, with id collisions corrupting updates (subagent task ids are small integers just like the parent's).
- **Placeholder merge on late id resolution.** If a `TaskUpdate` for id N arrives before the corresponding create's result resolves that id, the placeholder and the real task are merged into one row instead of leaving two rows sharing an id (one of which would be unreachable).
- **`TodoWrite` supersedes incremental state.** A session mixing both task dialects no longer flip-flops between two unrelated task sets: the full-list `TodoWrite` clears the incremental tracker.
