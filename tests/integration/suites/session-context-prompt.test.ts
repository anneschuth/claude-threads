...
describe.skipIf(SKIP)('Context Prompt', () => {
  ...
  describe('Mid-Thread Session Start', () => {
    it('should show context prompt when starting session mid-thread with 2+ messages', async () => {
      ...
      await startSessionMidThread(ctx, rootId, 'Help me fix this issue', config.mattermost.bot.username);
    ...
    it('should include context when user selects number option', async () => {
      ...
      // Start session mid-thread
      await startSessionMidThread(ctx, rootId, 'Help diagnose this', config.mattermost.bot.username);
    ...
    it('should skip context when user reacts with X', async () => {
      ...
      // Start session mid-thread
      await startSessionMidThread(ctx, rootId, 'Start fresh without context', config.mattermost.bot.username);
    ...
    it('should auto-include single message without prompting', async () => {
      ...
      // Start session mid-thread
      await startSessionMidThread(ctx, rootId, 'Help fix this bug', config.mattermost.bot.username);
      ...
      const hasContextPrompt = posts.some((p) =>
        p.user_id === ctx.botUserId && /Include thread context/i.test(p.message)
      );
  ...
  describe('Context Options', () => {
    it('should show appropriate options based on message count', async () => {
      ...
      // Start session mid-thread
      await startSessionMidThread(ctx, rootId, 'Analyze this issue', config.mattermost.bot.username);
